...
#Self-Attention is replaced with ConvGRU
import os
import glob
import numpy as np
...
class CostMemoryDecoderWithSkip(nn.Module):
    def __init__(self, hidden_dim=256, out_channels=3):
        ...
        # Multi-stage decoder with skip connections
    ...
    def forward(self, cost_query, keys, values, flow_init, h_prev, skips):
...
class FlowFormerForecastModel(nn.Module):
    ...
    def forward(self, x):  # ✅ PROPERLY INDENTED INSIDE THE CLASS
        B, T, C, H, W = x.shape
        h, lowres_flow = None, None
        ...
        for t in range(T):
            ...
            output_rgb, h = self.decoder(cost_query, keys, values, lowres_flow, h, skips)
            lowres_flow = self.decoder.flow_head(h)

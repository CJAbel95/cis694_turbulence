...
class TurbulenceForecastDataset(Dataset):
    def __init__(self, folder, seq_len=5, gap=20, size=256):
        ...
        max_required = seq_len + gap - 1
        if len(self.paths) < max_required:
            print(f"[WARNING] Not enough images. Reducing gap from {gap} to 1.")
            self.gap = 1
        self.valid_length = max(0, len(self.paths) - (self.seq_len + self.gap - 1))
    ...
    def __getitem__(self, idx):
        ...
        target_idx = idx + self.seq_len + self.gap - 1
        target_img = Image.open(self.paths[target_idx]).convert('RGB').resize((self.size, self.size))
        target_img = np.array(target_img).astype(np.float32) / 255.0
        target_img = target_img.transpose(2, 0, 1)
        return torch.tensor(np.stack(input_imgs), dtype=torch.float32), torch.tensor(target_img, dtype=torch.float32)


# === Encoder ===
class FullCostMemoryEncoder(nn.Module):
    def __init__(self, in_channels=3, embed_dim=256, heads=4, dim_head=64):
        super().__init__()

        # Patchification layers separated to extract skip connections
        ...
        # Key-Value projections for attention
        ...
        # Stack of spatial transformer blocks (e.g., AGT)
        self.transformer_stack = nn.Sequential(
            *[SpatialTransformerBlock(dim=embed_dim, heads=heads, dim_head=dim_head) for _ in range(3)]
        )

    def forward(self, x):
        # Apply patchification while capturing intermediate feature maps
        ...
        # Apply attention to spatially flattened features
        B, C, H, W = feat.shape
        ...
        # Reshape back to [B, C, H, W] and apply transformer stack
        T = tokens.permute(0, 2, 1).reshape(B, C, H, W)
        cost_memory = self.transformer_stack(T)

        # Return the skip connections in decoder order (coarse to fine)
        return feat, K, V, [skip1, skip2, feat]  # decoder will use skips[2], skips[1], skips[0]


# === Supporting Transformer Block ===
...
# === Flow Warping Function ===
def flow_warp(x, flow):
    """
    Warp `x` (feature map) using `flow` via bilinear sampling.
    Args:
        x    : (B, C, H, W) feature map
        flow : (B, 2, H, W) flow field
    Returns:
        warped_x : warped feature map
    """
    B, C, H, W = x.size()
    # Create mesh grid
    ...
    grid = torch.stack((x_grid, y), dim=0).unsqueeze(0).repeat(B, 1, 1, 1)  # (B, 2, H, W)
    ...
    warped_x = F.grid_sample(x, warped_grid, align_corners=True)

    return warped_x

# === Decoder ===
class CostMemoryDecoderWithSkip(nn.Module):
    def __init__(self, hidden_dim=256, out_channels=3, iters=12):
        super().__init__()

        self.iters = iters

        self.attn = CostAttention(hidden_dim)
        self.convgru = ConvGRUCell(input_dim=hidden_dim * 2, hidden_dim=hidden_dim)
        self.transformer = SpatialTransformerBlock(dim=hidden_dim)
        self.flow_head = nn.Conv2d(hidden_dim, 2, kernel_size=3, padding=1)

        # Projection to fix channel mismatch (258 → 256)
        self.cost_query_proj = nn.Conv2d(hidden_dim + 2, hidden_dim, kernel_size=1)
    ...
    def forward(self, q, keys, values, flow_init, h_prev, skips):
        """
        Recurrent flow refinement with warping.
        Args:
            q         : Query features from encoder (B, C, H, W)
            keys      : Attention keys (B, C, H, W)
            values    : Attention values (B, C, H, W)
            flow_init : Initial flow estimate (B, 2, H, W)
            h_prev    : Initial hidden state (B, C, H, W)
            skips     : List of skip connections for decoder
        Returns:
            final prediction map, hidden state, refined flow
        """
        flow = torch.zeros_like(flow_init) if flow_init is None else flow_init
        h = torch.zeros_like(h_prev) if h_prev is None else h_prev

        for _ in range(self.iters):
            # Warp query features using current flow
            warped_q = flow_warp(q, flow)

            # Build and project cost query
            cost_query_raw = torch.cat([warped_q, flow], dim=1)   # [B, 258, H, W]
            cost_query = self.cost_query_proj(cost_query_raw)     # [B, 256, H, W]

            # Attention-based context from memory
            context = self.attn(cost_query, keys, values)

            # Update memory with ConvGRU and Transformer
...
class FlowFormerForecastModel(nn.Module):
    def __init__(self, encoder_dim=256, hidden_dim=256, out_channels=3):
        super().__init__()
        self.encoder = FullCostMemoryEncoder(in_channels=3, embed_dim=encoder_dim)
        self.decoder = CostMemoryDecoderWithSkip(hidden_dim=hidden_dim, out_channels=out_channels)

    def forward(self, x):  # x: [B, T, C, H, W]
        B, T, C, H, W = x.shape
        h, lowres_flow, flow = None, None, None  # initialize flow as well
        ...
        for t in range(T):
            ...
            output_rgb, h, flow = self.decoder(cost_query, keys, values, lowres_flow, h, skips)
            lowres_flow = self.decoder.flow_head(h)  # preserve backward compatibility

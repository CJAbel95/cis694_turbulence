...
nx = nz = 256
x_points = np.linspace(30.2186, 1000 , nx)
y = 13
z_points = np.linspace(0, 240, nz)
...
for t in T_list:
    ...
    points = np.array([axis.ravel() for axis in np.meshgrid(x_points, [y], z_points, indexing='ij')], dtype=np.float64).T
    try:
        result = getData(dataset, variable, t, temporal_method, spatial_method, spatial_operator, points)
        u_field = np.array(result[0])[:, 0].reshape((nx, nz))

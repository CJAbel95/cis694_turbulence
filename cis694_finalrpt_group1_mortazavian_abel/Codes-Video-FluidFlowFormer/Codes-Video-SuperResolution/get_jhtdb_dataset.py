...
# === PARAMETERS ===
save_tsv_file = False
output_tsv_file = 'turbulence_out_dp'
...
for i, t in enumerate(T_list):
    ...
    while not getdata_success:
        ...
        try:
            ...
            dpdx_field = np.array(result[0])[:, 0].reshape((nx, ny))
            dpdy_field = np.array(result[0])[:, 1].reshape((nx, ny))
            # slices.append(ux_field)

            # Save ux to image
            plt.imsave(f"{save_img_dir}/dpdx/dpdx_t{t:.3f}.png", dpdx_field, cmap='seismic', vmin=-3, vmax=3)
            plt.imsave(f"{save_img_dir}/dpdy/dpdy_t{t:.3f}.png", dpdy_field, cmap='seismic', vmin=-3, vmax=3)
            # Save ux to image

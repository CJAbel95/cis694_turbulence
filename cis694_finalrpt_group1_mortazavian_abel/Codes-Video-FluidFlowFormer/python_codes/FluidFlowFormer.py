...
# Full hybrid loss (Perceptual + SSIM + Gradient)
...
for epoch in range(epochs):
    total_loss = 0
    for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs}"): ## Used for visual tracking of progress (not essential)
        inputs, target = batch
        ...
        # Compute hybrid loss directly
        loss = criterion(pred, target)  # Includes perceptual + SSIM + gradient loss
